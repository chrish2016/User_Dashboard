...
class Message(Model):
    def __init__(self):
        ...
        self.queries = {
            ...
            'fetch_messages_by_user_id':"SELECT messages.id as message_id, messages.message, users.id as user_id, users2.name as message_author, group_concat(comments.comment SEPARATOR '----') as msg_comments, group_concat(users3.name SEPARATOR '----') as comment_author FROM messages LEFT JOIN users ON users.id = messages.user_id LEFT JOIN users AS users2 ON users2.id = messages.author_id LEFT JOIN comments ON comments.message_id = messages.id LEFT JOIN users AS users3 ON users3.id = comments.user_id WHERE users.id = :id GROUP BY messages.id",
        }

    def create_comment(self, form_data, user_id):
        query = self.queries['create_comment']

        print user_id, form_data['message_id'], form_data['comment']
